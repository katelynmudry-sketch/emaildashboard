...
export default function DetailPanel({ email, onClose, onArchive, onMarkRead, onSaveDraft }: Props) {
        ...
        {archived ? (
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 text-sm py-2 border border-zinc-200 text-zinc-600 hover:text-zinc-800 hover:bg-zinc-50 rounded-lg transition-colors"
            >
              Mark read
            </button>
            <button
              onClick={() => setShowDraft(v => !v)}
              className="flex-1 text-sm py-2 bg-violet-50 border border-violet-200 text-violet-700 hover:bg-violet-100 rounded-lg transition-colors font-medium"
            >
            ...
            <button
              ...
              disabled={archiving}
              className="flex-1 text-sm py-2 bg-zinc-800 hover:bg-zinc-900 disabled:opacity-50 text-white rounded-lg transition-colors font-medium"
            >
              {archiving ? "…" : "Archive"}
            </button>
          </div>
